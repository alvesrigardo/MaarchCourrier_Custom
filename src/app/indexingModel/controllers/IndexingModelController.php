...
class IndexingModelController
{
        ...
        foreach ($fields as $key => $value) {
            ...
            if ($value['identifier'] == 'destination') {
                ...
                if ($fields[$key]['allowedValues'] == IndexingModelController::ALLOWED_VALUES_ALL_DOCTYPES) {
                    ...
                    $fields[$key]['allowedValues'] = array_column(DoctypeModel::get(['select' => ['type_id']]),
                        'type_id');
                } else {
                ...
                foreach ($fields[$key]['default_value'] as $itemKey => $item) {
                    if ($item['type'] == 'entity') {
                        ...
                        $user = UserModel::getById(
                            ['id' => $item['id'], 'select' => ['firstname', 'lastname', 'status']]
                        );
                        ...
                        if ($item['mode'] == 'dest' && !empty($destination) && !in_array($destination, $userEntities) &&
                            $destination != '"#myPrimaryEntity"') {
                            unset($fields[$key]['default_value'][$itemKey]);
                            continue;
                        }
                        if (!empty($user) && !in_array($user['status'], ['SPD', 'DEL'])) {
                            $fields[$key]['default_value'][$itemKey]['labelToDisplay'] = $user['firstname'] . ' ' .
                                $user['lastname'];
                            $fields[$key]['default_value'][$itemKey]['descriptionToDisplay'] = UserModel::getPrimaryEntityById(
                                ['id' => $item['id'], 'select' => ['entities.entity_label']]
                            )['entity_label'];
                        } else {
                            unset($fields[$key]['default_value'][$itemKey]);
                        }
    ...
    public function create(Request $request, Response $response): Response
    {
        ...
        } elseif (!Validator::stringType()->notEmpty()->validate($body['category'] ?? null) ||
            ...
            foreach ($fieldsMaster as $field) {
                ...
                foreach ($body['fields'] as $value) {
                    if (($field['identifier'] == 'destination' && $value['identifier'] == 'diffusionList')
                        ||
                        ($value['identifier'] == $field['identifier'] && $value['mandatory'] == $field['mandatory'] &&
        ...
        if (PrivilegeController::hasPrivilege(['privilegeId' => 'admin_indexing_models', 'userId' => $GLOBALS['id']])) {
            ...
            if (!empty($body['default'])) {
                IndexingModelModel::update(
                    ['set' => ['"default"' => 'false'], 'where' => ['"default" = ?'], 'data' => ['true']]
                );
            }
            $body['default'] = empty($body['default']) ? 'false' : 'true';
        } elseif (PrivilegeController::hasPrivilege(
            ['privilegeId' => 'create_public_indexing_models', 'userId' => $GLOBALS['id']]
        )) {
            $body['private'] = empty($body['private']) ? 'false' : 'true';
            $body['default'] = 'false';
        } else {
        ...
        foreach ($body['fields'] as $field) {
            ...
            if (strpos($field['identifier'], 'indexingCustomField_') !== false && !empty($field['default_value']) &&
                ...
                $customField = CustomFieldModel::getById(
                    ['id' => $customFieldId, 'select' => ['type'], 'where' => ['mode = ?'], 'data' => ['form']]
                );
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        $childrenModels = IndexingModelModel::get(
            ['select' => ['id', 'label'], 'where' => ['master = ?'], 'data' => [$args['id']]]
        );
            ...
            foreach ($childrenModels as $child) {
                    ...
                    foreach ($childFields as $value) {
                        if (($field['identifier'] == 'destination' && $value['identifier'] == 'diffusionList')
                            || ($value['identifier'] == $field['identifier'] &&
                                $value['mandatory'] == $field['mandatory'] && $value['unit'] == $field['unit'] &&
                ...
                foreach ($fieldsToKeep as $field) {
                    ...
                    IndexingModelFieldModel::create([
                        ...
                        'default_value'  => !isset($field['default_value']) ? null : json_encode(
                            $field['default_value']
                        ),
                        'unit'           => $field['unit'],
                        'allowed_values' => !isset($field['allowedValues']) ? null : json_encode(
                            $field['allowedValues']
                        ),
                    ]);
                }
        ...
        $allResourcesUsingModel = ResModel::get(
            ['select' => ['res_id'], 'where' => ['model_id = ?'], 'data' => [$args['id']]]
        );
        ...
        if (!empty($allResourcesUsingModel)) {
            $oldFieldList = IndexingModelFieldModel::get(
                ['select' => ['identifier'], 'where' => ['model_id = ?'], 'data' => [$args['id']]]
            );
            ...
            ResController::resetResourceFields(
                ['oldFieldList' => $oldFieldList, 'newFieldList' => $newFieldList, 'modelId' => $args['id']]
            );
            ...
            if (in_array('senders', $fieldsToDelete)) {
                ResourceContactModel::delete(
                    ['where' => ['res_id in (?)', 'mode = ?'], 'data' => [$allResourcesUsingModel, 'sender']]
                );
            }
            if (in_array('recipients', $fieldsToDelete)) {
                ResourceContactModel::delete(
                    ['where' => ['res_id in (?)', 'mode = ?'], 'data' => [$allResourcesUsingModel, 'recipient']]
                );
            }
    ...
    public function delete(Request $request, Response $response, array $args): Response
    {
        ...
        $model = IndexingModelModel::getById(
            ['select' => ['owner', 'private', '"default"', 'label', 'master'], 'id' => $args['id']]
        );
        if (empty($model)) {
            return $response->withStatus(400)->withJson(['errors' => 'Model not found']);
        } elseif (!empty($model['master']) && $model['private'] && $model['owner'] != $GLOBALS['id']) {
            return $response->withStatus(400)->withJson(['errors' => 'Model out of perimeter']);
        } elseif (!empty($model['master']) && empty($model['private']) && !PrivilegeController::hasPrivilege(
                ['privilegeId' => 'create_public_indexing_models', 'userId' => $GLOBALS['id']]
            )) {
            return $response->withStatus(400)->withJson(['errors' => 'Model out of perimeter']);
        } elseif (empty($model['master']) &&
        ...
        if (!empty($resources)) {
            ...
            if (!Validator::notEmpty()->intVal()->validate($body['targetId'])) {
                return $response->withStatus(400)->withJson(
                    ['errors' => 'Model is used by at least one resource', 'lang' => 'modelUsedByResources']
                );
            }
            ...
            $newFieldList = IndexingModelFieldModel::get(
                ['select' => ['identifier'], 'where' => ['model_id = ?'], 'data' => [$body['targetId']]]
            );
            ...
            if (in_array('senders', $fieldsToDelete)) {
                ResourceContactModel::delete(
                    ['where' => ['res_id in (?)', 'mode = ?'], 'data' => [$resources, 'sender']]
                );
            }
            if (in_array('recipients', $fieldsToDelete)) {
                ResourceContactModel::delete(
                    ['where' => ['res_id in (?)', 'mode = ?'], 'data' => [$resources, 'recipient']]
                );
            }
            if (in_array('tags', $fieldsToDelete)) {
